...
func ReadCookies(path string, cookies *map[string]string) error {
	...
	case "firefox":
		...
		cookiesSeq = firefox.TraverseCookies(path, kooky.DomainHasSuffix("bilibili.com"), kooky.Name("bili_jct")).OnlyCookies()
		bili_jct := getCookies(cookiesSeq)
		(*cookies)["buvid3"] = buvid.Value
		(*cookies)["SESSDATA"] = session_data.Value
		(*cookies)["DedeUserID"] = dede_user_id.Value
		(*cookies)["bili_jct"] = bili_jct.Value
	...
	case "chrome", "chromium", "edge":
		...
		dede_user_id := getCookies(cookiesSeq)
		cookiesSeq = chrome.TraverseCookies(path, kooky.DomainHasSuffix("bilibili.com"), kooky.Name("bili_jct")).OnlyCookies()
		bili_jct := getCookies(cookiesSeq)
		(*cookies)["buvid3"] = buvid.Value
		(*cookies)["SESSDATA"] = session_data.Value
		(*cookies)["DedeUserID"] = dede_user_id.Value
